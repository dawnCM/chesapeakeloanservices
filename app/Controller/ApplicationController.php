...
class ApplicationController extends AppController {
	...
	public function beforeFilter() {
		...
		$this->Session->write('Application.Campaign_id', '5a0a89236d95a');
		$this->Session->write('Application.Campaign_key', 'FMr7j4RxhPT9wBCZ3JYm');
	    ...
	    $this->Cookie->path = '/';
	    $this->Cookie->domain = 'chesapeakeloanservices.xyz';
	    ...
	    //$this->Cookie->write('pop_up', null, false, 120);
	...
	public function index() {
		...
		$this->set('LoanAmountPayday', array(
					'200'=>'$200', '300'=>'$300', '400'=>'$400', '500'=>'$500', '600'=>'$600', '700'=>'$700',
					'750'=>'$750', '800'=>'$800', '900'=>'$900', '1000'=>'$1,000'));
		...
		//print_r($this->Session->write('Application.Step',1));
	...
	public function processLead(){
		...
		if($this->request->is('ajax')){
			...
			if($this->Application->validates(array('fieldList' => array('BankAccountType','BankRoutingNumber','BankAccountNumber','BankName','BankTime')))) {
				...
				$response_array['url'] = "?sitename=chesapeakeloanservices&firstname=".$firstname."&lastname=".$lastname."&email=".$email."&address=".$address."&city=".$city."&state=".$state."&zipcode=".$zipcode;
	...
	public function setSessionDataAjax(){
		...
		if($this->request->is('ajax')){			
			...
			//}
			//print_r($this->Session->read('Application.Step'));
			if($this->Session->read('Application.Step') != 7){
				foreach($this->request->data as $key=>$value){
					$this->Session->write('Application.'.$key, $value);
				}
			...
			if($this->Session->read('Application.Step') == 3){
				...
				//$this->Cookie->write('pop_up', 'done', false, 120);
			}
