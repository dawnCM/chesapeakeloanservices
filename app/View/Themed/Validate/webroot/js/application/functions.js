...
//Page 2 - Application :: start
	if(jQuery('#basicWizard').length>0){
		...
		jQuery('#basicWizard').bootstrapWizard({
			...
			onNext: function(tab, navigation, index) {
				...
				if(index == 5){
					ga('send', 'event', 'Application', 'submission', 'Loan Application');
					saveSessioncb();
				}else{
					saveSession();
				}
	...
	jQuery('#Email').on('change', function(){
  		if(jQuery('#Email').parsley().validate() == true && jQuery('#FirstName').parsley().validate() == true && jQuery('#LastName').parsley().validate() == true){
  			...
  			//send2Leadbyte();
  			sendValidationCode();
  			jQuery('#ValidateEmail').val('');
  			jQuery("label[for='Email']").append(' <span class="validationsent alert-success">Validation Code Sent...</span>')
  		}else{
	...
	function sendValidationCode(){
		sender = {email:jQuery('#Email').val(),firstname:jQuery('#FirstName').val(),lastname:jQuery('#LastName').val()}
		
		jQuery.ajax({
			url: 'http://stage.chesapeakeloanservices.com/application/sendValidationCode',
			method:'post',
			data:sender,
			headers:{
	...
	function verifyValidationCode(){
		jQuery.ajax({
			url: 'http://stage.chesapeakeloanservices.com/application/verifyValidationCode/'+jQuery('#ValidateEmail').val(),
			method:'POST',
			async:false,
			headers:{
				'x-keyStone-nonce': nonce
